...
#
# El enunciado exige 2 nodos CPU + 2 nodos GPU.
# Usamos un hetjob (Slurm 22.05.2) para pedir 2 de 'batch' y 2 de 'GPU'.
# -----------------------------------------------------------------------
...
# Para hetjobs, Slurm define variables por grupo (HET_GROUP_0, HET_GROUP_1)
# o las variables legadas PACK_GROUP_0, PACK_GROUP_1.
NODES_0="${SLURM_JOB_NODELIST_HET_GROUP_0:-${SLURM_JOB_NODELIST_PACK_GROUP_0:-}}"
NODES_1="${SLURM_JOB_NODELIST_HET_GROUP_1:-${SLURM_JOB_NODELIST_PACK_GROUP_1:-}}"

if [ -z "${NODES_0}" ] || [ -z "${NODES_1}" ]; then
    echo "[civicmesh] ERROR: No se detectaron los grupos del hetjob. ¿Se usó #SBATCH hetjob?" >&2
    ...
    CPU_HOSTS=($(scontrol show hostnames "${NODES_0}"))
    GPU_HOSTS=($(scontrol show hostnames "${NODES_1}"))
    CPU_HOST_0="${CPU_HOSTS[0]}"
    CPU_HOST_1="${CPU_HOSTS[1]}"
    GPU_HOST_0="${GPU_HOSTS[0]}"
    GPU_HOST_1="${GPU_HOSTS[1]}"
fi
...
# Slurm copia el script a un spool temporal, así que $0 no sirve para rutas relativas.
# Usamos SLURM_SUBMIT_DIR que apunta al directorio desde donde se hizo el sbatch.
...
launch_remote() {
    ...
    local HET_ARG=""
    if [ -n "${HET_GROUP}" ]; then
        HET_ARG="--het-group=${HET_GROUP}"
    fi

    # Intentar con srun --overlap (requiere Slurm >= 20.11)
    srun --nodes=1 --ntasks=1 --overlap --nodelist="${NODE}" ${HET_ARG} \
        bash -c "${CMD}" \
        > "${LOG_FILE}" 2>&1 &
}
...
for COMUNA in "${CPU0_COMUNAS[@]}"; do
    ...
    launch_remote "${CPU_HOST_0}" "${RUN_DIR}/logs/${PEER_ID}.out" "${PEER_CMD}" "0"
done
...
for COMUNA in "${CPU1_COMUNAS[@]}"; do
    ...
    launch_remote "${CPU_HOST_1}" "${RUN_DIR}/logs/${PEER_ID}.out" "${PEER_CMD}" "0"
done
